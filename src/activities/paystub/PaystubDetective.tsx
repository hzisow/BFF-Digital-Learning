...
function gradeFor(score: number): [title: string, emoji: string, blurb: string] {
  if (score >= 90)
    return ['Chief Paystub Inspector', '🕵️', 'Payroll departments fear you. Nothing gets past that magnifying glass.']
  if (score >= 70)
    return ['Sharp-Eyed Auditor', '🔎', 'You caught most of the funny business — your future paychecks are in good hands.']
  if (score >= 45)
    return ['Rookie Detective', '🧢', 'You sniffed out some errors — keep practicing that hours × rate math.']
  return ['Payroll Got Away With It', '😅', 'The errors slipped by this time. Run it back — your money is worth the double-check.']
}
...
function statusChip(status: LineStatus): { text: string; classes: string } | null {
  switch (status) {
    case 'caught':
      return { text: `Caught! +${POINTS_PER_CATCH}`, classes: 'bg-green-100 text-green-700' }
    case 'missed':
      return { text: 'Missed error', classes: 'bg-red-100 text-red-700' }
    case 'false':
      return { text: `False alarm −${FALSE_ALARM_PENALTY}`, classes: 'bg-amber-100 text-amber-700' }
    case 'twin':
      return { text: 'Same catch ✓', classes: 'bg-green-100 text-green-700' }
    case 'clean':
      return null
  }
}
...
export default function PaystubDetective({ onComplete }: LiveGameProps) {
  ...
  // ---------- Final screen ----------
  if (phase === 'done') {
    const [title, emoji, blurb] = gradeFor(score)
    return (
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="card mt-4 animate-slide-up border-bff-200 bg-bff-50">
          <h2 className="font-display text-lg font-bold text-slate-900">
            <span aria-hidden="true">💡</span> The key takeaway
          </h2>
          <p className="mt-1 text-sm font-semibold text-slate-700">
            Always check your paystub — errors are common and it's YOUR money.
          </p>
          <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-700">
            <li>Multiply your hours × your rate. It should equal gross pay, every time.</li>
            <li>Know the fixed rates: Social Security is 6.2% and Medicare is 1.45% of gross.</li>
            <li>Question any fee you didn't agree to in writing — and any tax listed twice.</li>
            <li>Net pay must equal gross pay minus deductions. It can never be bigger than gross.</li>
          </ul>
        </div>
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">
            <span aria-hidden="true">🧾🔍</span> Paystub Detective
          </h1>
      ...
      <div className="card mt-4">
        <p className="text-sm text-slate-700">
          ...
          <strong>Case #{roundIndex + 1}: {round.worker}</strong> — {round.story}
        </p>
      </div>

      {/* The paystub */}
      <section className="card mt-4 p-0" aria-label={`Paystub for ${round.worker}`}>
        <div className="rounded-t-2xl border-b border-slate-200 bg-slate-100 px-5 py-4">
          ...
          <p className="text-sm text-slate-600">
            Employee: {round.worker} · Pay period: {round.period}
          </p>
        </div>
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              ...
                              {line.detail && <p className="text-xs text-slate-600">{line.detail}</p>}
                            </div>
      ...
      {revealing ? (
        <div className="card mt-4 animate-pop-in text-center" role="status">
          <h2 className="font-display text-lg font-bold text-slate-900">
            Round {roundIndex + 1} verdict
          </h2>
          <p className="mt-1 text-sm text-slate-700">
            You caught {roundResult.found} of {round.errorCount} error
            {round.errorCount === 1 ? '' : 's'}
            ...
            <strong className="text-bff-700">
              {roundResult.points >= 0 ? `+${roundResult.points}` : roundResult.points} points
            </strong>
            .
          </p>
          <button className="btn-primary mt-4" onClick={nextRound}>
            {roundIndex + 1 >= ROUNDS.length ? 'See your detective rating' : 'Next case'}{' '}
        ...
        <div className="mt-6 text-center">
          ...
          {selected.size === 0 && (
            <p className="mt-2 text-sm text-slate-500">
              Flag at least one suspicious line to submit. ({round.errorCount} error
              {round.errorCount === 1 ? '' : 's'} hiding in this stub.)
            </p>
          )}
        </div>
      )}
